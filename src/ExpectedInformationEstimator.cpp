...
double ExpectedInformationEstimator::EvaluateMIS(const VectorXd& design)
{
  ...
  for (int i = 0; i < N; ++i) {
    ...
    /*
     *  postCor = postCov;
     *  for (int ii = 0; ii < experiment->inputDim; ++ii) {
     *  for (int jj = 0; jj < experiment->inputDim; ++jj) {
     *   postCor(ii, jj) = postCov[i](ii, jj) / sqrt(postCov[i](ii, ii) * postCov[i](jj, jj));
     *  }
     *  }
     *
     *  double maxCorr = 0.9;
     ...
     *  for (int ii = 0; ii < experiment->inputDim; ++ii) {
     *  for (int jj = 0; jj < experiment->inputDim; ++jj) {
     *   if (ii != jj && fabs(postCor(ii, jj)) > maxCorr) {
     *     rescale = true;
     *     postCov(ii, jj) = copysign(maxCorr * sqrt(postCov[i](ii, ii) * postCov[i](jj, jj)), postCov[i](ii, jj));
     *   }
     *  }
     *  }
     *  if (rescale) rescaled++;
