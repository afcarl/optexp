...
int main(int argc, char **argv)
{
  ...
  if (experimentType == "linear") {
    ...
    experiment = make_shared<SimpleExperiment>(dim, poi, sigeps);
  ...
  cout << "design = " << design.transpose() << endl;
  ...
  estimator->N                       = GetOption<int>(argc, argv, "--N", 100);
  ...
  estimator->useMIS                  = GetOption<bool>(argc, argv, "--useMIS", false);
